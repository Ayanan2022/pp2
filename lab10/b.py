...
yourpassword = '1234'  # Ensure to secure your password appropriately


def insert_from_csv(file_path):
    conn = psycopg2.connect(f"dbname=testdb user=postgres password={yourpassword}")
    cur = conn.cursor()
        ...
        for row in reader:
            try:
                if len(row) == 3:
                    ...
                    print(f"Пропущена строка: {row} (не хватает данных)")
